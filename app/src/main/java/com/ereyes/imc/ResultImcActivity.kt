...
class ResultImcActivity : AppCompatActivity() {
    ...
    private fun initUI(result: Double) {
        when(result){
            in 0.00..18.50 -> {
                dataUI(result, R.string.bajo_peso, R.string.bajo_peso_description)
            }
            in 18.51..24.99 -> {
                dataUI(result, R.string.normal_peso, R.string.normal_peso_description)
            }
            in 25.00..29.99 -> {
                dataUI(result, R.string.sobrepeso, R.string.sobrepreso_description)
            }
            in 30.00..99.00 -> {
                dataUI(result, R.string.obesidad, R.string.obesidad_description)
            }
            else -> {
                dataUI(0.0, R.string.text_error, R.string.text_error)
            }
        }
    }

    private fun dataUI(result: Double, resultIMCText: Int, description: Int){
